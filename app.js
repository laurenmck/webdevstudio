...
const updatePage = async () => {
  ...
  const sortfruit = fruitsArray.filter(fruits => fruits.nutritions.sugar > 15)

  const newfruit = sortfruit.map(fruit => fruit.name).join(", ");
  // TODO: Create a new HTML element to display your data
  const newlst = document.createElement('div');
  newlst.innerHTML =  newfruit;


  // TODO: Append your new element to the page
  const existingElement = document.getElementById('cs1300-gallery');
  existingElement.append(newlst);
}
